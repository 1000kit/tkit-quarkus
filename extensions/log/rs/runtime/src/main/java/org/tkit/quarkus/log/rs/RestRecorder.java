...
public class RestRecorder {
    ...
    public void init(RestRuntimeConfig config, RestServiceValue values) {
        CONFIG = config;
        if (config.enabled() && config.regex().enabled()) {
        ...
        if (config.payload().enabled() && config.payload().regex().enabled()) {
    ...
    public static class RestConfig {
        ...
        private final boolean payloadRegexEnabled;

        public RestConfig() {
            this(false, false);
        }

        public RestConfig(boolean regexEnabled, boolean payloadRegexEnabled) {
            this.regexEnabled = regexEnabled;
            this.payloadRegexEnabled = payloadRegexEnabled;
        }
        ...
        public boolean isPayloadRegexEnabled() {
            return payloadRegexEnabled;
        }
    }
}
